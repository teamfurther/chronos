...
use Chronos\Traits\ContentManagement;
...
class ContentController extends Controller
{
            ...
            return response()->json([
                'alerts' => [
                    (object) [
                        ...
                        'message' => trans('chronos::alerts.:type successfully deleted.', ['type' => $type->name]),
                    ]
                ],
                'status' => 200
            ], 200);
            ...
            return response()->json([
                'alerts' => [
                    (object) [
                        'type' => 'error',
                        'title' => trans('chronos::alerts.Error.'),
                        'message' => trans('chronos::alerts.:type deletion was unsuccessful.', ['type' => $type->name]),
                    ]
                ],
                'status' => 500
            ], 500);
            ...
            return response()->json([
                'alerts' => [
                    (object)[
                        'type' => 'success',
                        'title' => trans('chronos::alerts.Success.'),
                        'message' => trans_choice('chronos::alerts.:count items deleted.', $deleted_content_count, ['count' => $deleted_content_count])
                    ]
                ],
                'status' => 200
            ], 200);
    ...
    public function export(Request $request, ContentType $type)
    {
        ...
        foreach ($content as $key => $item) {
            ...
            $fieldsets = ContentFieldset::where('parent_type', 'Chronos\Models\Content')->where('parent_id', $item->id)->get();
            ...
            foreach ($item->all_fieldsets as $fieldset) {
                foreach ($fieldset->fields as $field) {
                    $generator->addIndent(2);
                    if ($fieldset->parent_type == 'Chronos\Models\ContentType') {
        ...
        return response()->json([
            'alerts' => [
                (object) [
                    'type' => 'success',
                    'title' => trans('chronos::alerts.Success.'),
                    'message' => trans('chronos::alerts.:type successfully created.', ['type' => $type->name]),
                ]
            ],
            'content' => $content,
            'status' => 200
    ...
    public function translate(Request $request, ContentType $type, Content $content)
    {
        ...
        $fieldsets = ContentFieldset::where('parent_id', $content->id)->where('parent_type', 'Chronos\Models\Content')->get();
        foreach ($fieldsets as $fieldset) {
        ...
        $fields_data = ContentFieldData::whereHas('field', function($q1) {
            $q1->whereHas('fieldset', function($q2) {
                $q2->where('parent_type', 'Chronos\Models\ContentType');
            });
        ...
        return response()->json([
            'alerts' => [
                (object) [
                    'type' => 'success',
                    'title' => trans('chronos::alerts.Success.'),
                    'message' => trans('chronos::alerts.:type successfully updated.', ['type' => $type->name]),
                ]
            ],
            'content' => $content,
            'status' => 200
