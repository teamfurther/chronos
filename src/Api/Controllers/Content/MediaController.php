...
class MediaController extends Controller
{
    ...
    public function index(Request $request)
    {
        $itemsPerPage = $request->has('perPage')
            ...
            : Config::get('chronos.media_items_per_page');
    ...
    public function destroy(Media $media)
    {
        ...
        if (!isset($pathinfo['path'])) {
            ...
            foreach (config('chronos.upload_paths') as $p) {
            ...
            return response()->json([
                'alerts' => [
                    (object)[
                        ...
                        'message' => trans('chronos::alerts.File successfully deleted.'),
                    ]
                ],
                'status' => 200
            ], 200);
            ...
            return response()->json([
                'alerts' => [
                    (object)[
                        ...
                        'message' => trans('chronos::alerts.File deletion was unsuccessful.'),
                    ]
                ],
                'status' => 500
            ], 500);
            ...
            return response()->json([
                'alerts' => [
                    (object)[
                        'type' => 'success',
                        'title' => trans('chronos::alerts.Success.'),
                        'message' => trans_choice('chronos::alerts.:count items deleted.', $deleted_media_count, ['count' => $deleted_media_count])
                    ]
                ],
                'status' => 200
            ], 200);
    ...
    public function store(Request $request)
    {
        if ($request->has('files')) {
            ...
            foreach ($request->get('files') as $key => $file) {
                ...
                $paths = Config::get('chronos.upload_paths');
                ...
                if (file_put_contents($filepath, $data) === false) {
                    ...
                    $alerts[] = (object) [
                        'type' => 'error',
                        'title' => trans('chronos::alerts.Error.'),
                        'message' => trans('chronos::alerts.Error when uploading :file.', ['file' => $basename]),
                    ];
                ...
                $alerts[] = (object) [
                    'type' => 'success',
                    'title' => trans('chronos::alerts.Success.'),
                    'message' => trans('chronos::alerts.:file successfully uploaded.', ['file' => $basename]),
                ];
            }
                    ...
                    return response()->json([
                        'alerts' => [
                            (object) [
                                'type' => 'success',
                                'title' => trans('chronos::alerts.Success.'),
                                'message' => trans('chronos::alerts.:count files successfully uploaded.', ['count' => $alerts_counter['success']]),
                            ]
                        ],
                        'status' => 200
                    ], 200);
                    ...
                    return response()->json([
                        'alerts' => [
                            (object) [
                                'type' => 'error',
                                'title' => trans('chronos::alerts.Error.'),
                                'message' => trans('chronos::alerts.:count files failed to upload.', ['count' => $alerts_counter['error']]),
                            ]
                        ],
                        'status' => 200
                    ], 200);
                    ...
                    return response()->json([
                        'alerts' => [
                            (object) [
                                'type' => 'warning',
                                'title' => trans('chronos::alerts.Warning.'),
                                'message' => trans('chronos::alerts.:count_success file(s) successfully uploaded. :count_error file(s) failed to upload.', ['count_success' => $alerts_counter['success'], 'count_error' => $alerts_counter['error']]),
                            ]
                        ],
                        'status' => 200
                    ], 200);
            ...
            return response()->json([
                'alerts' => [
                    (object) [
                        'type' => 'error',
                        'title' => trans('chronos::alerts.Error.'),
                        'message' => trans('chronos::alerts.No files to upload.'),
                    ]
                ],
                'status' => 400
            ], 400);
        ...
        return response()->json([
            'alerts' => [
                (object) [
                    'type' => 'success',
                    'title' => trans('chronos::alerts.Success.'),
                    'message' => trans('chronos::alerts.Media successfully updated.'),
                ]
            ],
            'media' => $media,
            'status' => 200
