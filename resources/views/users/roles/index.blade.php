...
@section('content')
    <header class="subheader">
        <h1>{!! trans('chronos::interface.Roles') !!}</h1>
        <ul class="breadcrumbs">
            <li><span class="icon c4icon-pencil-3"></span></li>
            <li class="active">{!! trans('chronos::interface.Roles') !!}</li>
        </ul>
        @can ('add_roles')
        <div class="main-action create">
            <a data-placement="left" data-tooltip="tooltip" title="{!! trans('chronos::interface.Create role') !!}" data-toggle="modal" data-target="#create-role-dialog">{!! trans('chronos::interface.Create role') !!}</a>
        </div>
        @endcan
    </header><!--/.subheader -->
                            ...
                            <table class="table table-condensed" v-cloak>
                                ...
                                <tbody v-show="!dataLoader && data.length !== 0">
                                    <tr v-for="item in data">
                                        ...
                                        <td>
                                            @can('edit_roles')<a class="marginR15" data-toggle="modal" data-target="#update-role-dialog" v-on:click="setModalURL('#update-role-dialog', item.endpoints.update); setValue('#update-role-dialog input[name=name]', item.name); setValue('#update-role-dialog input[name=label]', item.label);">{!! trans('chronos::interface.Edit') !!}</a>@endcan
                                            @can('delete_roles')<a data-toggle="modal" data-target="#delete-role-dialog" v-on:click="setdeleteURL(item.endpoints.destroy, $event)">{!! trans('chronos::interface.Delete') !!}</a>@endcan
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                <role-editor inline-template>
                    {!! Form::open(['route' => 'api.users.roles.store', 'method' => 'POST', 'v-on:submit.prevent' => 'saveForm', 'novalidate' => 'novalidate']) !!}
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title">{!! trans('chronos::interface.Create role') !!}</h4>
                        </div>
            ...
            <div class="modal-content">
                <role-editor inline-template>
                    {!! Form::open(['url' => '', 'method' => 'PATCH', 'v-on:submit.prevent' => 'saveForm', 'novalidate' => 'novalidate']) !!}
                        <div class="modal-header">
                            <button type="button" class="modal-close" data-dismiss="modal"><span class="icon c4icon-cross-2"></span></button>
                            <h4 class="modal-title">{!! trans('chronos::interface.Edit role') !!}</h4>
                        </div>
                        <div class="modal-body">
                            <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'name') }">
                                <label class="control-label label-req" for="name">{!! trans('chronos::forms.Name') !!}</label>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-danger">
                <form v-on:submit.prevent="deleteModelFromDialog">
                    <div class="modal-header">
                        <button type="button" class="modal-close" data-dismiss="modal"><span class="icon c4icon-cross-2"></span></button>
                        <h4 class="modal-title">{!! trans('chronos::interface.Delete role') !!}</h4>
                    </div>
                    <div class="modal-body">
