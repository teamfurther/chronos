...
@section('content')
    <header class="subheader">
        <h1>{!! trans('chronos::interface.Permissions') !!}</h1>
        <ul class="breadcrumbs">
            <li><span class="icon c4icon-pencil-3"></span></li>
            <li class="active">{!! trans('chronos::interface.Permissions') !!}</li>
        </ul>
    </header><!--/.subheader -->
    <div class="container">
        <div class="row">
            <div class="col-xs-12">
                <div class="panel">
                    <h2 class="panel-title">{!! trans('chronos::interface.Permissions') !!}</h2>
                    @if (count($roles) > 0)
                    ...
                    @else
                        <p class="no-results">{!! trans('chronos::interface.There are no roles yet. Please <a href=":link">add a role</a> first.', ['link' => route('chronos.users.roles')]) !!}</p>
                    @endif
                </div>
            </div>
        </div>
