...
@section('content')
    <header class="subheader">
        <h1>{!! trans('chronos::interface.Languages') !!}</h1>
        <ul class="breadcrumbs">
            <li><span class="icon c4icon-pencil-3"></span></li>
            <li class="active">{!! trans('chronos::interface.Languages') !!}</li>
        </ul>
        @if (settings('is_multilanguage'))
        <div class="main-action create">
            <a data-placement="left" data-tooltip="tooltip" title="{!! trans('chronos::interface.Add language') !!}" data-toggle="modal" data-target="#add-language-dialog">{!! trans('chronos::interface.Add language') !!}</a>
        </div>
        @endif
    </header><!--/.subheader -->
                        ...
                        <div class="panel">
                            <table class="table table-condensed" v-cloak>
                                ...
                                <tr v-for="item in data">
                                    ...
                                    <td>
                                        <a class="marginR15" v-on:click="ajaxGet(item.endpoints.activate, true)" v-if="!item.status">{!! trans('chronos::interface.Activate') !!}</a>
                                        <a class="marginR15" v-on:click="ajaxGet(item.endpoints.deactivate, true)" v-if="item.status">{!! trans('chronos::interface.Deactivate') !!}</a>
                                        <a data-toggle="modal" data-target="#delete-language-dialog" v-on:click="setdeleteURL(item.endpoints.destroy)">{!! trans('chronos::interface.Delete') !!}</a>
                                    </td>
                                </tr>
                                </tbody>
                            </table>
    ...
    @else
        <p class="no-results">{!! trans('chronos::interface.Multilanguage support is deactivated.') !!}</p>
    @endif
@endsection
                ...
                <language-editor inline-template>
                    {!! Form::open(['route' => 'api.settings.languages.store', 'method' => 'POST', 'v-on:submit.prevent' => 'saveForm', 'novalidate' => 'novalidate']) !!}
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title">{!! trans('chronos::interface.Add language') !!}</h4>
                        </div>
                        <div class="modal-body">
                            <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'code') }">
                                <label class="control-label label-req" for="code">{!! trans('chronos::forms.Select language') !!}</label>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-danger">
                <form v-on:submit.prevent="deleteModelFromDialog">
                    <div class="modal-header">
                        <button type="button" class="modal-close" data-dismiss="modal"><span class="icon c4icon-cross-2"></span></button>
                        <h4 class="modal-title">{!! trans('chronos::interface.Delete language') !!}</h4>
                    </div>
                    <div class="modal-body">
