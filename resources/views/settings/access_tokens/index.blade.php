...
@section('content')
    <header class="subheader">
        <h1>{!! trans('chronos::interface.Access tokens') !!}</h1>
        <ul class="breadcrumbs">
            <li><span class="icon c4icon-pencil-3"></span></li>
            <li class="active">{!! trans('chronos::interface.Access tokens') !!}</li>
        </ul>
        <div class="main-action create">
            <a data-placement="left" data-tooltip="tooltip" title="{!! trans('chronos::interface.Create access token') !!}" data-toggle="modal" data-target="#create-access-token-dialog">{!! trans('chronos::interface.Create access token') !!}</a>
        </div>
    </header><!--/.subheader -->
                        ...
                        <div class="panel">
                            <table class="table table-condensed" v-cloak>
                                ...
                                <tr v-for="item in data">
                                    ...
                                    <td><a data-toggle="modal" data-target="#view-access-token-modal" v-on:click="setInnerHTML('#view-access-token-modal .modal-title', item.name); setInnerHTML('#view-access-token-modal .token', item.access_token); " class="marginR15">{!! trans('chronos::interface.View') !!}</a> @can('delete_access_tokens')<a data-toggle="modal" data-target="#delete-access-token-dialog" v-on:click="setdeleteURL(item.endpoints.destroy)">{!! trans('chronos::interface.Delete') !!}</a>@endcan</td>
                                </tr>
                                </tbody>
                            </table>
            ...
            <div class="modal-content">
                <access-token-editor inline-template>
                    ...
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">{!! trans('chronos::interface.Create access token') !!}</h4>
                    </div>
                    <div class="modal-body">
                        <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'name') }">
                            ...
                            <span class="help-block" v-else><span>{!! trans('chronos::forms.The name of the access token must be unique.') !!}</span></span>
                        </div>
                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-danger">
                <form v-on:submit.prevent="deleteModelFromDialog">
                    <div class="modal-header">
                        <button type="button" class="modal-close" data-dismiss="modal"><span class="icon c4icon-cross-2"></span></button>
                        <h4 class="modal-title">{!! trans('chronos::interface.Delete access token') !!}</h4>
                    </div>
                    <div class="modal-body">
    ...
    <div class="modal fade" id="view-access-token-modal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-danger">
                ...
                <div class="modal-body">
                    <p class="marginT15"><strong>{!! trans('chronos::interface.Access token') !!}</strong></p>
