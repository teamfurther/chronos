<script type="text/x-template" id="fieldset-template">
    <div class="fieldset" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'fieldsets.' + order), minimized: isMinimized }">
        <div class="reorder" v-on:mousedown="dragStart" v-on:mouseout="dragEnd"><div class="drag"></div><span v-html="name" v-if="name" v-on:click="focusNameField('fieldsets.' + order + '.name')"></span><em v-if="!name" v-on:click="focusNameField('fieldsets.' + order + '.name')">{!! trans('chronos::forms.Unnamed fieldset') !!}</em></div>
        ...
        <div class="row">
            <div class="col-xs-12 col-sm-6">
                <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'fieldsets.' + order + '.name') }">
                    <label class="control-label label-req" v-bind:for="'fieldsets.' + order + '.name'">{!! trans('chronos::forms.Fieldset name') !!}</label>
            ...
            <div class="col-xs-12 col-sm-6">
                <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'fieldsets.' + order + '.fieldset_machine') }" v-show="(machineName != '' && !machineNameChanged) || machineNameChanged || Object.hasKey(store.formErrors, 'fieldsets.' + order + '.fieldset_machine')">
                    <label class="control-label label-req" v-bind:for="'fieldsets.' + order + '.fieldset_machine'">{!! trans('chronos::forms.Machine name') !!}</label>
        ...
        <div class="form-group">
            <label class="control-label" v-bind:for="'fieldsets.' + order + '.description'">{!! trans('chronos::forms.Description') !!}</label>
        ...
        <div class="form-group">
            <label class="control-label" v-bind:for="'fieldsets.' + order + '.repeatable'">{!! trans('chronos::forms.Repeatable?') !!}</label>
            <div class="checkbox">
                <label>
                    <input v-bind:id="'fieldsets.' + order + '.repeatable'" v-bind:name="'fieldsets[' + order + '][repeatable]'" v-model="repeatable" type="checkbox" />
                    {!! trans('chronos::forms.Yes') !!}
                </label>
            </div>
        </div>

        <label class="control-label">{!! trans('chronos::forms.Fields') !!}</label>
        ...
        <a class="btn btn-action add-field" v-on:click="addField">{!! trans('chronos::forms.Add field') !!}</a>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content modal-danger">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">{!! trans('chronos::interface.Delete fieldset') !!}</h4>
                    </div>
                    <div class="modal-body">
                        <p class="marginT15 text-center"><strong>{!! trans('chronos::interface.WARNING! All content of belonging to this fieldset will be deleted as well. This action is irreversible.') !!}</strong></p>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-default" type="button" data-dismiss="modal">{!! trans('chronos::interface.Close') !!}</button>
                        <button class="btn btn-danger" type="button" v-on:click="deleteFieldset(true)">{!! trans('chronos::interface.Delete') !!}</button>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/x-template" id="field-template">
    <div class="field" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'fieldsets.' + fieldset.order + '.fields.' + order), minimized: isMinimized }">
        <div class="reorder" v-on:mousedown="dragStart" v-on:mouseout="dragEnd"><div class="drag"></div><span v-html="name" v-if="name" v-on:click="focusNameField('fieldsets.' + fieldset.order + '.fields.' + order + '.name')"></span><em v-if="!name" v-on:click="focusNameField('fieldsets.' + fieldset.order + '.fields.' + order + '.name')">{!! trans('chronos::forms.Unnamed field') !!}</em></div>
        <a v-bind:class="{minimize: !isMinimized, maximize: isMinimized}" v-on:click="toggleMinimize"></a>
        ...
        <div class="row">
            <div class="col-xs-12 col-sm-6">
                <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'fieldsets.' + fieldset.order + '.fields.' + order + '.name') }">
                    <label class="control-label label-req" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.name'">{!! trans('chronos::forms.Field name') !!}</label>
            ...
            <div class="col-xs-12 col-sm-6">
                <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'fieldsets.' + fieldset.order + '.fields.' + order + '.field_machine') }" v-show="(machineName != '' && !machineNameChanged) || machineNameChanged || Object.hasKey(store.formErrors, 'fieldsets.' + fieldset.order + '.fields.' + order + '.field_machine')">
                    <label class="control-label label-req" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.field_machine'">{!! trans('chronos::forms.Machine name') !!}</label>
        ...
        <div class="row">
            <div class="col-xs-12 col-sm-6">
                <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'fieldsets.' + fieldset.order + '.fields.' + order + '.type') }">
                    <label class="control-label label-req" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.type'">{!! trans('chronos::forms.Type') !!}</label>
            ...
            <div class="col-xs-12 col-sm-6">
                <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'fieldsets.' + fieldset.order + '.fields.' + order + '.widget') }">
                    <label class="control-label label-req" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.widget'">{!! trans('chronos::forms.Widget') !!}</label>
        ...
        <div class="row">
            <div class="col-xs-12 col-sm-6" v-if="['file', 'image'].indexOf(type) == -1">
                <div class="form-group">
                    <label class="control-label" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.default'">{!! trans('chronos::forms.Default value') !!}</label>
            ...
            <div class="col-xs-12 col-sm-6">
                <div class="form-group">
                    <label class="control-label" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.repeatable'">{!! trans('chronos::forms.Repeatable?') !!}</label>
                    <div class="checkbox">
                        <label>
        ...
        <div class="row" v-if="type == 'image'">
            <div class="col-xs-12 col-sm-6">
                <div class="form-group">
                    <label class="control-label" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.enable_alt'">{!! trans('chronos::forms.Enable "alt" field?') !!}</label>
                    <div class="checkbox">
                        <label>
            ...
            <div class="col-xs-12 col-sm-6">
                <div class="form-group">
                    <label class="control-label" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.enable_title'">{!! trans('chronos::forms.Enable "title" field?') !!}</label>
                    <div class="checkbox">
                        <label>
        ...
        <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'fieldsets.' + fieldset.order + '.fields.' + order + '.entity_type') }" v-if="type == 'entity'">
            <label class="control-label label-req" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.entity_type'">{!! trans('chronos::forms.Entity type') !!}</label>
            <select class="form-control" v-bind:id="'fieldsets.' + fieldset.order + '.fields.' + order + '.entity_type'" v-bind:name="'fieldsets[' + fieldset.order + '][fields][' + order + '][entity_type]'" v-model="entityType">
                <option></option>
                <optgroup label="{!! trans('chronos::interface.Content types') !!}" v-if="entityTypes.contentTypes">
                    <option v-for="entity in entityTypes.contentTypes" v-bind:value="entity.model + ':' + entity.id" v-html="entity.name"></option>
                </optgroup>
                <optgroup label="{!! trans('chronos::interface.Users') !!}" v-if="entityTypes.userRoles">
                    <option value="\App\Models\User">{!! trans('chronos::interface.All users') !!}</option>
        ...
        <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'fieldsets.' + fieldset.order + '.fields.' + order + '.values') }" v-if="type == 'list'">
            <label class="control-label label-req" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.values'">{!! trans('chronos::forms.Values') !!}</label>
            ...
            <span class="help-block" v-else>{!! trans('chronos::forms.List of possible values, one per line, in "key => value" format.') !!}</span>
        </div>
        <div class="row" v-if="type == 'number'">
            <div class="col-xs-12 col-sm-6">
                <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'fieldsets.' + fieldset.order + '.fields.' + order + '.step') }">
                    <label class="control-label" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.step'">{!! trans('chronos::forms.Step') !!}</label>
        ...
        <div class="form-group">
            <label class="control-label" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.help_text'">{!! trans('chronos::forms.Help text') !!}</label>
            ...
            <span class="help-block">{!! trans('chronos::forms.Instructions to be presented to the user on the content editing form.') !!}</span>
        </div>
        <div class="form-group" v-if="widget != 'media'">
            <label class="control-label" v-bind:for="'fieldsets.' + fieldset.order + '.fields.' + order + '.rules'">{!! trans('chronos::forms.Validation rules') !!}</label>
            ...
            <span class="help-block">{!! trans('chronos::forms.Enter validation rules separated by a "|" (pipe) character.') !!}</span>
        </div>
        ...
        <div class="modal fade" v-bind:id="'delete-field-dialog-' + id" tabindex="-1" role="dialog" v-if="id">
            <div class="modal-dialog" role="document">
                <div class="modal-content modal-danger">
                    <div class="modal-header">
                        <button type="button" class="modal-close" data-dismiss="modal"><span class="icon c4icon-cross-2"></span></button>
                        <h4 class="modal-title">{!! trans('chronos::interface.Delete field') !!}</h4>
                    </div>
                    <div class="modal-body">
                        <p class="marginT15 text-center"><strong>{!! trans('chronos::interface.WARNING! All content of belonging to this field will be deleted as well. This action is irreversible.') !!}</strong></p>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-default" type="button" data-dismiss="modal">{!! trans('chronos::interface.Close') !!}</button>
                        <button class="btn btn-danger" type="button" v-on:click="deleteField(true)">{!! trans('chronos::interface.Delete') !!}</button>
                    </div>
                </div>
            </div>
        </div>
