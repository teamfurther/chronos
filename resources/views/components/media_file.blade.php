    ...
    <div class="media-file">
        <div class="clearfix" v-if="src">
            ...
            <div class="pull-left" v-bind:class="{ paddingT30: !isImage }">
                ...
                <a class="marginR15" v-on:click="openMediaDialog">{!! trans('chronos::forms.Change file') !!}</a>
                <a class="text-danger" v-on:click="removeFile">{!! trans('chronos::forms.Remove file') !!}</a>
                <div class="form-group marginT15" v-if="isImage && enableAlt">
                    <label class="control-label">{!! trans('chronos::forms.Alt tag') !!}<input class="form-control" v-bind:name="name + '[alt]'" type="text" v-model="alt" /></label>
                </div>
                <div class="form-group" v-if="isImage && enableTitle">
                    <label class="control-label">{!! trans('chronos::forms.Title tag') !!}<input class="form-control" v-bind:name="name + '[title]'" type="text" v-model="title" /></label>
                </div>
            </div>
        </div>

        <a data-toggle="modal" data-target="#select-file-dialog" v-on:click="openMediaDialog(true)" v-if="!src">{!! trans('chronos::forms.Select file') !!}</a>
