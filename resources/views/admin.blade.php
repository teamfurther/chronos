...
<html lang="en">
<head>
    <title>{!! trans('chronos::interface.Administration') !!} | {{ env('APP_NAME', 'Your Chronos Site') }}</title>
    ...
    <div class="header-left">
        ...
        ><div class="dropdown user-account">
            ...
            <ul class="dropdown-menu user-account-dropdown" role="menu" aria-labelledby="user-account-toggle">
                <li><a href="{{ route('chronos.auth.profile') }}"><span class="icon c4icon-user-profile"></span>{!! trans('chronos::interface.My profile') !!}</a></li>
                <li><a href="{{ route('chronos.auth.logout') }}"><span class="icon c4icon-power"></span>{!! trans('chronos::interface.Log out') !!}</a></li>
            </ul>
        </div>
    </div>
...
<script>
    Vue.http.interceptors.push(function(request, next) {
        request.headers.set('Authorization', 'Bearer {{ config("chronos.access_token") }}');
