...
@section('content')
    <header class="subheader">
        <h1>{!! trans('chronos::interface.Content types') !!}</h1>
        <ul class="breadcrumbs">
            <li><span class="icon c4icon-pencil-3"></span></li>
            <li class="active">{!! trans('chronos::interface.Content types') !!}</li>
        </ul>
        @can ('add_content_types')
        <div class="main-action create">
            <a data-placement="left" data-tooltip="tooltip" title="{!! trans('chronos::interface.Create content type') !!}" data-toggle="modal" data-target="#create-content-type-dialog">{!! trans('chronos::interface.Create content type') !!}</a>
        </div>
        @endcan
    </header><!--/.subheader -->
                            ...
                            <table class="table table-condensed" v-cloak>
                                <thead>
                                    <tr>
                                        ...
                                        <th>
                                            <div class="bulk-actions">
                                                ...
                                                <ul class="dropdown-menu">
                                                    ...
                                                    <li><a data-toggle="modal" data-target="#delete-content-types-dialog">{!! trans('chronos::interface.Delete') !!}</a></li>
                                                    @endcan
                                                    @can ('export_content_types')
                                                    <li><a v-on:click="performBulkAction('{{ route('api.content.types.export') }}', 'DOWNLOAD', 'types', $event)">{!! trans('chronos::interface.Export') !!}</a></li>
                                                    @endcan
                                                </ul>
                                            </div>
                                        </th>
                                        ...
                                        <th><sortable field="items_count">{!! trans('chronos::interface.Items') !!}</sortable></th>
                                ...
                                <tbody v-show="!dataLoader && data.length !== 0">
                                    <tr v-for="item in data">
                                        ...
                                        <td>
                                            @can ('edit_content_types')<a v-bind:href="item.admin_urls.edit" class="marginR15">{!! trans('chronos::interface.Edit') !!}</a>@endcan
                                            @can ('edit_content_type_fieldsets')<a v-bind:href="item.admin_urls.edit_fieldsets" class="marginR15">{!! trans('chronos::interface.Edit fieldsets') !!}</a>@endcan
                                            @can ('delete_content_types')<a data-toggle="modal" data-target="#delete-content-type-dialog" v-on:click="setdeleteURL(item.endpoints.destroy, $event)">{!! trans('chronos::interface.Delete') !!}</a>@endcan
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
            ...
            <div class="modal-content">
                <content-type-editor inline-template>
                    ...
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">{!! trans('chronos::interface.Create content type') !!}</h4>
                    </div>
                    <div class="modal-body">
                        <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'name') }">
                            <label class="control-label label-req" for="name">{!! trans('chronos::forms.Name') !!}</label>
                            ...
                            <span class="help-block" v-else>{!! trans('chronos::forms.The human-readable name of this content type. This text will be displayed as part of the list on the <em>Create content</em> page. Must be unique.') !!}</span>
                        </div>
                        <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'title_label') }">
                            <label class="control-label label-req" for="title_label">{!! trans('chronos::forms.Title field label') !!}</label>
                        ...
                        <div class="form-group">
                            <div class="checkbox">
                                <label>
                                    ...
                                    {!! trans('chronos::forms.Translatable?') !!}
                                </label>
                            </div>

                            <span class="help-block">{!! trans('chronos::forms.Specifies whether this content type will be available in multiple languages.') !!}</span>
                        </div>
                        @endif
                        <div class="form-group" v-bind:class="{ 'has-error': Object.hasKey(store.formErrors, 'notes') }">
                            <label class="control-label" for="notes">{!! trans('chronos::forms.Notes') !!}</label>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-danger">
                <form v-on:submit.prevent="deleteModelFromDialog">
                    <div class="modal-header">
                        <button type="button" class="modal-close" data-dismiss="modal"><span class="icon c4icon-cross-2"></span></button>
                        <h4 class="modal-title">{!! trans('chronos::interface.Delete content type') !!}</h4>
                    </div>
                    <div class="modal-body">
                        <p class="marginT15 text-center"><strong>{!! trans('chronos::interface.WARNING! All content of this type will be deleted as well. This action is irreversible.') !!}</strong></p>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-default" type="button" data-dismiss="modal">{!! trans('chronos::interface.Close') !!}</button>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-danger">
                <form v-on:submit.prevent="performBulkAction('{{ route('api.content.types.destroy_bulk') }}', 'DELETE', 'types', $event)">
                    <div class="modal-header">
                        <button type="button" class="modal-close" data-dismiss="modal"><span class="icon c4icon-cross-2"></span></button>
                        <h4 class="modal-title">{!! trans('chronos::interface.Delete content types') !!}</h4>
                    </div>
                    <div class="modal-body">
